...
def graph_config():
    ...
    plt.rcParams.update({"axes.grid" : False, "axes.facecolor": "#021e37", 'text.color': 'w', 
...
def silhouette_analysis(X, labels, ax, k):
    ...
    for i in np.unique(labels):
        ...
        ax.set_title(f'Silhouette Analysis - N clusters = {k}', fontdict={'size': 20})
        y_lower = y_upper + 10
        ax.axes.grid(True, color='gray', linewidth=0.5)
...
def kmeans_performance(X, clusters, title='', metric='euclidean', plot=True):
    ...
    if plot:
        plt.plot(clusters, kmeans_list,  marker='o', c='#108372')
        plt.xlabel('Number of clusters')
        plt.ylabel('Silhouette Score')
        plt.title(title + ' Silhouette Score')
        return None
...
def gmm_performance(X, components,  title='', metric='euclidean', covariance_type='full', plot=True):
    ...
    if plot:
        plt.plot(components, gmm_list, marker='o', c='#108372')
        plt.xlabel('Number of clusters')
        plt.ylabel('Silhouette Score')
        plt.title(title + ' Silhouette Score')
        return None
...
def hierarchical_performance(X, clusters, title='', metric='euclidean', plot=True):
    ...
    if plot: 
        ...
        plt.plot(clusters, agg_list,  marker='o', c='#108372')
        plt.xlabel('Number of clusters')
        plt.ylabel('Silhouette Score')
        plt.title(title + ' Silhouette Score')
...
def qq_plot(feature, df, std):
    
    #plt.style.use('ggplot')

    n = df[feature].shape[0]

    standard_feat = std.fit_transform(df[feature].values.reshape(-1, 1))
    ...
    sns.scatterplot(x=norm_quants, y=quants, color='#021e37')